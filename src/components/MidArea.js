...
const MidArea = () => {
  ...
  return (
    <div className="flex-1 h-full overflow-auto bg-gray-900 p-6" onDrop={handleDrop} onDragOver={handleDragOver}>
      <div className="bg-black rounded-lg shadow-md p-6">

        {selectedSprite ? (
          <>
            <div className='flex gap-4'>
              <h2 className="text-2xl flex font-bold text-white mb-4">Actions for Selected Sprite
              </h2>
              <div className='text-sm'><SpriteImage spriteName={selectedSprite.name} styles={{ width: "60px", height: "60px" }} /></div>
            </div>
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-gray-400 mb-2">Current Actions:</h3>
              <ul className="space-y-2">
                {selectedSprite.actions.map((action, index) => (
                  <li key={index} className="bg-gray-600 text-white px-3 py-1 flex justify-between align-middle rounded-full text-sm">
